...
function MealDetailIngredients({initialIngredients, servings, setServings}: MealDetailIngredientsProps) {
    const colorScheme = useColorScheme();
    const [showIngredientsInfo, setShowIngredientsInfo] = useState(false);
    const [checkedIngredients, setCheckedIngredients] = useState<Record<string, boolean>>({});
    const [showConvertModal, setShowConvertModal] = useState<boolean>(false);
    const {ingredients} = useIngredients({defaultIngredients: initialIngredients, servings});
        ...
        <>
            ...
            >
                <View style={styles.actionRow}>
                    <TouchableOpacity onPress={() => setServings(prevCount => prevCount + 1)}>
                        <Ionicons
                            name={'add-circle'}
                            size={36}
                    ...
                    <TouchableOpacity onPress={() => setServings(prevCount => Math.max(1, prevCount - 1))}
                                      disabled={servings === 1}>
                        <Ionicons
                            name={'remove-circle'}
                            size={36}
                ...
                <Button color={'red'} title="Reset" onPress={resetServings} disabled={servings === 1}/>
            </BottomSheetModal>
        </>
    );
}
